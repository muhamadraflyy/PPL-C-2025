...
export default function ServicePopularCard({
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        console.log("[ServicePopularCard] Status check error:", e);
      }
    };
  ...
  // Build thumbnail URL with comprehensive fallback
  ...
  const freelancerAvatarSrc = getImageUrl(service?.freelancer_avatar || service?.freelancerAvatar);
  const avatarFallbacks = getImageFallbacks(service?.freelancer_avatar || service?.freelancerAvatar);

  // Data mapping
  const badgeLabel = service?.nama_kategori || service?.category || "Website";
  const categoryLine = service?.highlight || service?.nama_kategori || service?.category || "UI/UX Desainer";
  const titleLine = service?.judul || service?.title || "Layanan";

  const freelancerName = service?.freelancer_name || service?.freelancer || "Freelancer";
  const price = Number(service?.harga ?? service?.price ?? 0);
  const rating = Number(service?.rating_rata_rata ?? service?.rating ?? 0);
  const reviews = Number(service?.jumlah_rating ?? service?.reviews ?? 0);
  ...
  const handleFavorite = async (e) => {
    ...
    if (!isClient) {
      console.log("[ServicePopularCard] User is not client, cannot favorite");
      return;
    }
    ...
    try {
      if (isFavorite) {
        ...
        if (res?.success) {
          setIsFavorite(false);
          onFavorite?.(service, false);
        }
      } else {
        const res = await favoriteService.addFavorite(service?.id);
        if (res?.success) {
          setIsFavorite(true);
          onFavorite?.(service, true);
        }
      }
    } catch (err) {
      console.error("[ServicePopularCard] Favorite error:", err);
    } finally {
  ...
  const handleBookmark = async (e) => {
    ...
    if (!isClient) {
      console.log("[ServicePopularCard] User is not client, cannot bookmark");
      return;
    }
    ...
    try {
      if (isBookmarked) {
        ...
        if (res?.success) {
          ...
          onBookmark?.(service, false);
        }
      } else {
        const res = await bookmarkService.addBookmark(service?.id);
        if (res?.success) {
          setIsBookmarked(true);
          onBookmark?.(service, true);
        }
      }
    } catch (err) {
      console.error("[ServicePopularCard] Bookmark error:", err);
    } finally {
